...
IE for a full install of cryptodredge or an update of all files run:
```
...
IE to only update the cryptodredge program to the latest release run:
```
...
IE to uninstall cryptodredge run:
...
## Config Sample
Here is an example of how to add cryptodredge to ethos. These instructions will apply to any miner simply by changing "ubqminer" to the miner you want.

### Set the miner
You can use the miner on your local config or globally on a remote config
```
globalminer cryptodredge
```
OR in a remote config to set just one rig
```
miner c94e13 cryptodredge
```
...
```
cryptodredge=proxywallet walletORusername
cryptodredge=proxypool1 pool.com:port
cryptodredge=proxypool2 pool.com:port
cryptodredge=poolpass1 x
cryptodredge=poolpass2 x
```
...
```
globalminer cryptodredge-single
```
