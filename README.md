...
IE for a full install of teamredminer or an update of all files run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) teamredminer install
```

### Updating

The miner can be easily updated when new releases are out by just changing the action argument to "update"
IE to only update the teamredminer program to the latest release run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) teamredminer update
```

### UnInstalling

The miner can be easily uninstalled by changing the action argument to "revert"
IE to uninstall teamredminer run:

```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) teamredminer revert
```

## Config Sample
Here is an example of how to add teamredminer to ethos. These instructions will apply to any miner simply by changing "teamredminer" to the miner you want.
...
```
teamredminer=proxywallet walletORusername
teamredminer=proxypool1 pool.com:port
teamredminer=proxypool2 pool.com:port
teamredminer=poolpass1 x
teamredminer=poolpass2 x
```
...
```
globalminer teamredminer-single
```
