...
IE for a full install of soliditysha3miner or an update of all files run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) soliditysha3miner install
```

### Updating

The miner can be easily updated when new releases are out by just changing the action argument to "update"
IE to only update the soliditysha3miner program to the latest release run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) soliditysha3miner update
```

### UnInstalling

The miner can be easily uninstalled by changing the action argument to "revert"
IE to uninstall soliditysha3miner run:

```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) soliditysha3miner revert
```

## Config Sample
Here is an example of how to add soliditysha3miner to ethos. These instructions will apply to any miner simply by changing "soliditysha3miner" to the miner you want.

### Set the miner
You can use the miner on your local config or globally on a remote config
```
globalminer soliditysha3miner
```
OR in a remote config to set just one rig
```
miner c94e13 soliditysha3miner
```
...
```
soliditysha3miner=proxywallet walletORusername
soliditysha3miner=proxypool1 pool.com:port
soliditysha3miner=proxypool2 pool.com:port
soliditysha3miner=poolpass1 x
soliditysha3miner=poolpass2 x
```
...
```
globalminer soliditysha3miner-single
```
