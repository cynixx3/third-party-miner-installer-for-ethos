...
IE for a full install of kbminer or an update of all files run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) kbminer install
```

### Updating

The miner can be easily updated when new releases are out by just changing the action argument to "update"
IE to only update the kbminer program to the latest release run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) kbminer update
```

### UnInstalling

The miner can be easily uninstalled by changing the action argument to "revert"
IE to uninstall kbminer run:

```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) kbminer revert
```

## Config Sample
Here is an example of how to add kbminer to ethos. These instructions will apply to any miner simply by changing "kbminer" to the miner you want.
...
```
kbminer=proxywallet walletORusername
kbminer=proxypool1 pool.com:port
kbminer=proxypool2 pool.com:port
kbminer=poolpass1 x
kbminer=poolpass2 x
```
