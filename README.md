...
IE for a full install of grin-miner or an update of all files run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) grin-miner install
```

### Updating

The miner can be easily updated when new releases are out by just changing the action argument to "update"
IE to only update the grin-miner program to the latest release run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) grin-miner update
```

### UnInstalling

The miner can be easily uninstalled by changing the action argument to "revert"
IE to uninstall grin-miner run:

```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) grin-miner revert
```

## Config Sample
Here is an example of how to add grin-miner to ethos. These instructions will apply to any miner simply by changing "grin-miner" to the miner you want.

### Set the miner
You can use the miner on your local config or globally on a remote config
```
globalminer grin-miner
```
OR in a remote config to set just one rig
```
miner c94e13 grin-miner
```
...
```
grin-miner=proxywallet walletORusername
grin-miner=proxypool1 pool.com:port
grin-miner=proxypool2 pool.com:port
grin-miner=poolpass1 x
grin-miner=poolpass2 x
```
