...
IE for a full install of energiminer or an update of all files run:
```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) energiminer install
```
...
IE to uninstall energiminer run:

```
source <(curl -s https://raw.githubusercontent.com/cynixx3/third-party-miner-installer-for-ethos/master/miner-manager) energiminer revert
```

## Config Sample
Here is an example of how to add energiminer to ethos. These instructions will apply to any miner simply by changing "energiminer" to the miner you want.

### Set the miner
You can use the miner on your local config or globally on a remote config
```
globalminer energiminer
```
OR in a remote config to set just one rig
```
miner c94e13 energiminer
```
...
```
energiminer=proxywallet walletORusername
energiminer=proxypool1 pool.com:port
energiminer=proxypool2 pool.com:port
energiminer=poolpass1 x
energiminer=poolpass2 x
```
...
```
globalminer energiminer-single
```
