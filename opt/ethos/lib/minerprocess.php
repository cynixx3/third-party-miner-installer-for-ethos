...
function start_miner()
{
	...
	*  CCMINER/NEVERMORE
	********************************/
	if (preg_match("/(ccminer|nevermore|cryptodredge)/",$miner)) {
		...
		if($miner == "nevermore" && !preg_match("/-a/",$flags)) {
			$flags .= " -a x16r ";
                ...
                if($miner == "cryptodredge" && !preg_match("/-a/",$flags)) {
                        $flags .= " -a lyra2v3 ";
	...
	********************************/
    ...
    if(intval(trim(file_get_contents("/opt/ethos/etc/allow.file"))) == 0) {
		...
		$miner_params['cryptodredge'] = $flags ." ". $pools;
		...
		$miner_suffix['cryptodredge'] = " " . $mine_with . " " . $extraflags;
